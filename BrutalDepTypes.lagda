...
\begin{code}
    ...
    *-comm a (succ b) = sym ((cong (λ x → a + x) (*-comm b a)) ~ (lemma-*succ a b))
\end{code}
...
\begin{code}
  ...
  filter≡filterP₁ p (a ∷ as) with as
  filter≡filterP₁ p (a ∷ as) | [] with p a
  filter≡filterP₁ p (a ∷ as) | [] | true = refl
  filter≡filterP₁ p (a ∷ as) | [] | false = refl
  filter≡filterP₁ p (a ∷ as) | (b ∷ xs) with p a
  filter≡filterP₁ p (a ∷ as) | (b ∷ xs) | true with p b
  filter≡filterP₁ p (a ∷ as) | b ∷ xs | true | true = cong (λ x → a ∷ (b ∷ x)) (filter≡filterP₁ p xs)
  filter≡filterP₁ p (a ∷ as) | b ∷ xs | true | false = cong (λ x → a ∷ x) (filter≡filterP₁ p xs)
  filter≡filterP₁ p (a ∷ as) | (b ∷ xs) | false with p b
  filter≡filterP₁ p (a ∷ as) | b ∷ xs | false | true = cong (λ x → b ∷ x) (filter≡filterP₁ p xs)
  filter≡filterP₁ p (a ∷ as) | b ∷ xs | false | false = filter≡filterP₁ p xs

  filterN≡filterP : {A : Set} → (p : A → Bool) → (as : List A) → filterN p as ≡ filterP p as
  filterN≡filterP p as = sym (filter≡filterN p as) ~ filter≡filterP₁ p as
...
\begin{code}
  module ThrowAwayExercise where
    ...
    contradiction = λ a ¬a → ⊥-elim (¬a a)
   
    contraposition : ∀ {α β} {A : Set α} {B : Set β} → (A → B) → (¬ B → ¬ A)
    contraposition = λ a→b ¬b a → ¬b (a→b a)
...
module Data-ℕ where
  ...
  module ℕ-Op where
    ...
    +-comm : ∀ a b → a + b ≡ b + a
    +-comm zero b = sym (lemma-+zero b)
    +-comm (succ a) b rewrite +-comm a b = lemma-+succ b a
  ...
  module ℕ-RelOp where
    ...
    succ n ≡? succ m | no ¬a = no (¬a ∘ cong pred)
    ...
    succ n ≤? zero = no (λ ())
    succ n ≤? succ m with n ≤? m
    succ n ≤? succ m | yes a = yes (s≤s a)
    succ n ≤? succ m | no ¬a = no (¬a ∘ ≤-unsucc)
