...
int adaptive_quickhull_predicate(const point &a, const point &b, const point &c1, const point &c2)
{
    double p[16];
    p[0] = mul(b.x, c1.y, p[1]);
    p[2] = mul(-b.x, c2.y, p[3]);
    p[4] = mul(-a.x, c1.y, p[5]);
    p[6] = mul(a.x, c2.y, p[7]);
    p[8] = mul(-b.y, c1.x, p[9]);
    p[10] = mul(b.y, c2.x, p[11]);
    p[12] = mul(a.y, c1.x, p[13]);
    p[14] = mul(-a.y, c2.x, p[15]);

    expansion_sum<2, 2>(p, p + 2);
    expansion_sum<2, 2>(p + 4, p + 6);
    expansion_sum<2, 2>(p + 8, p + 10);
    expansion_sum<2, 2>(p + 12, p + 14);
    
    expansion_sum<4, 4>(p, p + 4);
    expansion_sum<4, 4>(p + 8, p + 12);

    expansion_sum<8, 8>(p, p + 8);
