...
\end{code}
В коде на Agda широко используются символы Unicode. Пример — определение типа
натуральных чисел.
\begin{code}
data ℕ : Set where
  zero : ℕ
...
Зависимые типы позволяют определять типы, зависящие (или индексированные) от значений
других типов. Пример — список, индексированный длиной:
\begin{code}
